{
  ...
  persistentStorageNames = filter (path: cfg.${path}.enable) (lib.attrNames cfg);
  ...
  config = {
    home.file = let
      ...
      mkLinkNameValuePair = persistentStorageName: fileOrDir: {
        name =
          if cfg.${persistentStorageName}.removePrefixDirectory
          then dirListToPath (tail (splitPath [fileOrDir]))
          else fileOrDir;
      ...
      mkLinksToPersistentStorage = persistentStorageName:
        lib.listToAttrs (
          ...
          (cfg.${persistentStorageName}.files
            ++ (map getDirPath
    ...
    systemd.user.services = let
      mkBindMountService = persistentStorageName: dir: let
        mountDir =
          if cfg.${persistentStorageName}.removePrefixDirectory
          then dirListToPath (tail (splitPath [dir]))
          else dir;
    ...
    home.activation = let
      ...
      reloadSystemd =
        if config.home.activation ? reloadSystemD
        then "reloadSystemD"
        else "reloadSystemd";

      mkBindMount = persistentStorageName: dir: let
        mountDir =
          if cfg.${persistentStorageName}.removePrefixDirectory
          then dirListToPath (tail (splitPath [dir]))
          else dir;
      ...
      mkUnmount = persistentStorageName: dir: let
        mountDir =
          if cfg.${persistentStorageName}.removePrefixDirectory
          then dirListToPath (tail (splitPath [dir]))
          else dir;
      ...
      mkLinkCleanup = persistentStorageName: dir: let
        mountDir =
          if cfg.${persistentStorageName}.removePrefixDirectory
          then dirListToPath (tail (splitPath [dir]))
          else dir;
